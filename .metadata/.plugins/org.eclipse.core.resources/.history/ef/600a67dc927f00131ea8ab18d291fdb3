...
public class MineView extends JPanel {
	...
	private void initData(int x, int y) {
		...
		for (int i = 0; i < mine_size; i++) {
			...
			if (p.y + 1 < y_size && p.x - 1 >= 0
					&& game[p.y + 1][p.x - 1] != -1)
				game[p.y + 1][p.x - 1] = game[p.y + 1][p.x - 1] + 1;

			if (p.y + 1 < y_size && p.x + 1 < x_size
					&& game[p.y + 1][p.x + 1] != -1)
				game[p.y + 1][p.x + 1] = game[p.y + 1][p.x + 1] + 1;

			if (p.y - 1 >= 0 && p.x + 1 < x_size
					&& game[p.y - 1][p.x + 1] != -1)
				game[p.y - 1][p.x + 1] = game[p.y - 1][p.x + 1] + 1;
	...
	private void initUI() {
		removeAll();
		setLayout(null);
		setSize(blank_size * x_size + marge_size * 2, blank_size * y_size
				+ marge_size * 2);
		...
		for (int i = 0; i < y_size; i++) {
			for (int j = 0; j < x_size; j++) {
				...
				b.setBounds(marge_size + j * blank_size, marge_size + i
						* blank_size, blank_size, blank_size);
