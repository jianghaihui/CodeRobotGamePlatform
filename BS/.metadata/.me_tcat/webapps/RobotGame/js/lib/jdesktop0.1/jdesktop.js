 ...
 *desktop需引入myDesktop.js创建命名空间，和jquery 及 jquery ui,contextMenu jquery插件
 -----------------------------------------------------------------------------*/
//声明desktop空间,封装相关操作
myDesktop.NS("desktop");
myDesktop.desktop={
    winWH:function(){
        ...
        jqElem
            .smartMenu(data,{
                ...
                afterShow:function(){
                    ...
                    var myData=myDesktop.desktop.getMydata(),
...
myDesktop.NS("desktop.win");
myDesktop.desktop.win={
    ...
    closeWin:function(obj){
        ...
        myDesktop.desktop.taskBar.delWinTab(obj);
    ...
    minimize:function(obj){
        ...
        if($win.size()>1){
            ...
            objTab=myDesktop.desktop.taskBar.findWinTab(obj);
    ...
    newWin:function(options){
        ...
        var myData=myDesktop.desktop.getMydata(),
            ...
            $windows=$("div.windows"),
            _this=this,
            curwinNum=myDesktop._is(myData.winNum,"Number")?myData.winNum:0;//判断当前已有多少窗口
        ...
        var wLeft=myDesktop._is(options['WindowPositionLeft'],"Number")?options['WindowPositionLeft']+dxy-panelLeft.left:(wh['w']-options['WindowWidth'])/2+dxy-panelLeft.left;
        var wTop=myDesktop._is(options['WindowPositionTop'],"Number")?options['WindowPositionTop']+dxy/2:(wh['h']-options['WindowHeight'])/2+dxy/2;
        ...
        if($("#"+id).size()<=0){
            //在任务栏里添加tab
            myDesktop.desktop.taskBar.addWinTab(options['WindowTitle'],options['WindowsId']);
            //初始化新窗口并显示
            ...
            //如果已存在此窗口，判断是否隐藏
            var wins=$("#"+id),objTab=myDesktop.desktop.taskBar.findWinTab(wins);
            if(wins.is(":hidden")){
                wins.show();
                objTab.removeClass('defaultTab').addClass('selectTab');//当只有一个窗口时
                myDesktop.desktop.win.switchZindex(wins);
            }else{
...
//侧边工具栏
myDesktop.NS("desktop.lrBar");
myDesktop.desktop.lrBar={
    upLrBar:function(){
        var myData=myDesktop.desktop.getMydata()
    ...
    init:function(iconData){
        ...
        //附加data数据
        myDesktop.desktop.iconDataInit(iconData);

        //如果窗口大小改变，则更新侧边栏位置
        $(window).wresize(function(){
            myDesktop.desktop.winWH();//更新窗口大小数据
            _this.upLrBar();
        });
        ...
        $default_app
            .droppable({
                ...
                drop: function(event,ui) {
                    ...
                    myDesktop.desktop.deskIcon.init();
                    _this.init();
                }
            })
            .find('li')
...
/*----------------------------------------------------------------------------------
 //声明任务栏空间，任务栏相关js操作
 ----------------------------------------------------------------------------------*/
myDesktop.NS("desktop.taskBar");
myDesktop.desktop.taskBar={
    timer:function(obj){
    ...
    contextMenu:function(tab,id){
        ...
        var data=[
            [{
                ...
                text:"最小化",
                func:function(){
                    myDesktop.desktop.win.minimize($("#myWin_"+tab.data('win')));
                }
            }]
            ,[{
                text:"关闭",
                func:function(){
                    ...
                    myDesktop.desktop.win.closeWin($("#myWin_"+tab.data('win')));
                }
            }]
        ];
        myDesktop.desktop.contextMenu(tab,data,"taskTab_menu"+id,10);
    },
            ...
            .click(function(){
                ...
                if(desk.is(".currDesktop")){
                    if(win.is(".hideWin")){
                        ...
                        myDesktop.desktop.win.switchZindex(win);
                    }else{
                        if($(this).hasClass('selectTab')){
                            myDesktop.desktop.win.minimize(win);
                        }else{
                            myDesktop.desktop.win.switchZindex(win);
                        }
                    }
...
//navbar
myDesktop.NS("desktop.navBar");
myDesktop.desktop.navBar={
    init:function(){
        var myData=myDesktop.desktop.getMydata()
            ,$navBar=myData.panel.navBar
        ...
        $navTab
            ...
            .click(function(event){
                ...
                event.stopPropagation();
                var i=$navTab.index($(this));
                myDesktop.desktop.deskIcon.desktopMove($innerPanel,$deskIcon,$navTab,500,desktopWidth+lBarWidth,i);
            });
    }
};

//桌面背景
myDesktop.NS("desktop.wallpaper");
myDesktop.desktop.wallpaper={
    init:function(imgUrl){

        //将当前窗口宽度和高度数据存储在body元素上
        myDesktop.desktop.winWH();
        ...
        if(imgUrl!==null){
            ...
            myDesktop.getImgWh(imgUrl,function(imgW,imgH){
                if(imgW<=winWh.w){
...
//桌面图标区域
myDesktop.NS("desktop.deskIcon");
myDesktop.desktop.deskIcon={
    //桌面图标排列
    ...
    init:function(iconData){
        ...
        myDesktop.desktop.contextMenu($(document.body),data,"body",10);
    }
}